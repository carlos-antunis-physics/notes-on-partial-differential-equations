...
>   Let $S = \left\{\mathbf{u}_i\right\}_{i\in I \subseteq \mathbb{N}}$ being a non-empty subset of $\mathbb{E}$, thus:
>
>   >   $\mathbf{u}$ is a linear combination of vectors in $S \Leftrightarrow \exists \left\{\alpha^i\right\}_{i\in I \subseteq \mathbb{N}} \subset \mathcal{F}: \mathbf{u} = \alpha^iu_i$
>
>   the notation $\alpha^i\mathbf{u}_i$ uses Einstein's sum convention.
