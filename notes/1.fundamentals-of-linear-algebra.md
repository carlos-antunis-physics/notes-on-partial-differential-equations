...
>   Let $S = \{$ $\mathbf{u}_{i}$ $\}_{i\in I \subseteq \mathbb{N}}$ being a non-empty subset of $\mathbb{E}$, thus:
>
>   >   $\mathbf{u}$ is a linear combination of vectors in $S \Leftrightarrow \exists \{$ $\alpha^i$ $\}_{i\in I \subseteq \mathbb{N}} \subset \mathcal{F}: \mathbf{u} = \alpha^iu_i$
>
